...
import { formatForApi, formatForUser } from '../utils/dateformat';
...
const DEFAULT_DESCRIPTION = 'API for AN0M-ARCHIVES application';
...
export class AppInfoService implements IAppInfoService {
    ...
    private loadPackageInfoSecurely(): SafePackageInfo {
        try {
            ...
            if (packageContent.length > 100000) {
                throw new Error('package.json file too large to process safely');
            }
            ...
            logger.warn('Unable to securely read the package.json file:', {
                error: error.message
            });
    ...
    public static getInstance(): IAppInfoService {
        if (!AppInfoService.instance) {
            try {
                ...
                logger.info('AppInfoService initialized successfully:', {
                    timestamp: formatForUser()
                });
            } catch (error: any) {
                logger.error('Error initializing AppInfoService:', {
                    error: error.message
                });
                throw new Error('Failed to initialize AppInfoService');
            }
        }
    ...
    public getUptime(): string {
        try {
            ...
            if (uptimeSeconds < 0) {
                logger.warn('Negative uptime detected, resetting to 0s');
                return '0s';
            }

            if (uptimeSeconds > MAX_UPTIME_DAYS * 24 * 60 * 60) {
                logger.warn('Excessive uptime detected, applying limit');
            ...
            logger.error('Error calculating uptime:', {
                error: error.message
            });
            return '0s';
        }
